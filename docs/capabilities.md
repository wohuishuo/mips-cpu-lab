...
保留上面的 C01–C12 硬件/课程能力，另增加五个可直接交互的能力。运行 `npm run lab`，
用浏览器打开本机实验台，无需 Vivado 或 MCP。

| 编号 | 垂直能力 | 交付 |
|---|---|---|
| V01 | 输入 → 指令 → 结果 | 可编辑汇编、A/B 输入、四个示例、错误定位 |
| V02 | 时钟 → 五级流水线 | 动画、单步、回退、级间状态、转发/停顿、动态指令轨迹 |
| V03 | 写回/访存 → 状态 | 32 寄存器、256 字节 RAM、字节布局与变化高亮 |
| V04 | 32 位运算 → 一位电路 | 全加器 XOR/AND/OR、逐位 Sum 与 Carry |
| V05 | 拼接 → 定制 CPU | 元件拖动、端口连线、DFF、真值表关卡、JSON、安装 CUSTOM |

[使用说明与软件清单](visual-lab.md) · [浏览器验证记录](visual-lab-verification.md)。
CUSTOM 是浏览器教学扩展，V01–V05 不扩大原 FPGA ISA 或原实板证据的范围。
